...
- Fork releases: [5 releases, June–August 2026](https://github.com/postgresql-tools/wal-g/releases).
...
- **`neon-drill`** — the same rehearsal as `restore-test`, but the recovered data is loaded into a throwaway [Neon](https://neon.tech) branch you can connect to and query, then deleted. Two-stage by necessity: Neon cannot receive a physical backup, so the drill restores physically to a scratch directory and dumps logically into the branch. The branch is removed on every exit path, interrupts included, and `neon-branches` lists any that outlived a dead host — [docs](docs/PostgreSQL.md#neon-drill)
